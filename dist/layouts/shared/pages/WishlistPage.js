                ...
                if (useAPI && isAuthenticated) {
                    ...
                    if (apiWishlist && apiWishlist.length > 0) {
                        const itemsWithProducts = apiWishlist
                            .filter((item) => item.product) // Only items with product data
                            .map((item) => transformProductToStoreProduct(item.product));
                        // Deduplicate items by product ID to prevent duplicates
                        const uniqueItems = itemsWithProducts.filter((item, index, self) => index === self.findIndex((t) => t.id === item.id));
