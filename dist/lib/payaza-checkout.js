 ...
 */
export function validateAndNormalizeCurrencyWithFallback(currency, fallback = 'USD') {
    try {
 ...
 */
export function getCurrencyFromItems(items, fallbackCurrency) {
 ...
 */
export const initiatePayazaCheckout = async (config) => {
    try {
        ...
        payazaCheckout.setCallback((callbackResponse) => {
 ...
 */
export const generateCheckoutConfig = (items, customerEmail, publicKey, options) => {
    ...
    if (options?.shippingAddress) {
        ...
        if (addr.address)
            shippingParts.push(addr.address);
        if (addr.city)
            shippingParts.push(addr.city);
        if (addr.state)
            shippingParts.push(addr.state);
        if (addr.zipCode)
            shippingParts.push(addr.zipCode);
        if (addr.country)
            shippingParts.push(addr.country);
    }
