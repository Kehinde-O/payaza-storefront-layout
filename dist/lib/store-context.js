...
export function StoreProvider({ children, storeSlug, initialConfig }) {
    const [store, setStore] = useState(initialConfig || null);
    ...
    useEffect(() => {
        if (typeof window === 'undefined' || !storeSlug || !store)
            return;
                ...
                try {
                    ...
                    if (isAuthenticated) {
                        try {
                            ...
                            if (apiWishlist) {
                                setWishlist(apiWishlist.map((item) => item.productId));
                            }
                        }
                        catch (wishlistError) {
                ...
                try {
                    ...
                    if (guestWishlist) {
                        ...
                        if (Array.isArray(productIds) && productIds.length > 0) {
                            ...
                            try {
                                const apiWishlist = await wishlistService.getWishlist();
                                const backendProductIds = apiWishlist.map((item) => item.productId);
                                ...
                                setWishlist(updatedWishlist.map((item) => item.productId));
    ...
    const addToCart = async (product, quantity = 1, variantId) => {
            ...
            if (shouldUseAPI(storeSlug || '')) {
                ...
                if (isAuthenticated) {
                    try {
                        ...
                        const newWishlist = items.map((item) => item.productId);
