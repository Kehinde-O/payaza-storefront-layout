        ...
        <div className="min-h-screen bg-white pb-20">
            ...
            <section data-section="hero" className="relative bg-gray-900 text-white py-20 overflow-hidden">
                <div className="absolute inset-0 opacity-20">
                    <Image
                        src={contactConfig?.contactHeader?.backgroundImage || "https://images.unsplash.com/photo-1596524430615-b46475ddff6e?q=80&w=2070&auto=format&fit=crop"}
                ...
                <div className="container mx-auto px-4 relative z-10 text-center">
                    <h1 className="text-4xl md:text-6xl font-bold mb-4 tracking-tight">
                        {contactConfig?.contactHeader?.title || "Get in Touch"}
                    </h1>
                    <p className="text-xl text-gray-300 max-w-2xl mx-auto font-light">
                        {contactConfig?.contactHeader?.subtitle || "We'd love to hear from you. Here's how you can reach us."}
                    </p>
                </div>
            </section>
                ...
                <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 flex flex-col lg:flex-row">
                    ...
                    <div className="lg:w-2/5 p-8 md:p-12 text-white relative overflow-hidden" style={{ backgroundColor: 'var(--store-primary)' }}>
                        ...
                        <h2 className="text-2xl font-bold mb-8 relative z-10">
                            {contactConfig?.contactForm?.infoTitle || "Contact Information"}
                        </h2>
                        <p className="mb-10 text-white/90 leading-relaxed relative z-10">
                            {contactConfig?.contactForm?.infoDescription || "Fill out the form and our team will get back to you within 24 hours."}
                        </p>
                        ...
                        {formState === 'success' ? (
                            ...
                            <form onSubmit={handleSubmit} className="relative z-10 space-y-6">
                                <h2 className="text-2xl font-bold text-gray-900 mb-6">
                                    {contactConfig?.contactForm?.title || "Send us a Message"}
                                </h2>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    {(contactConfig?.contactForm?.showPhone !== false) && (
                                        <div className="space-y-2">
                                ...
                                {(contactConfig?.contactForm?.showSubject !== false) && (
                                    <div className="space-y-2">
                                        ...
                                        <select
                                            ...
                                            className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:border-gray-900 focus:ring-0 outline-none transition-all"
                                        >
                                            {contactConfig?.contactForm?.subjects?.length > 0 ? (
                                                contactConfig.contactForm.subjects.map((s: any, i: number) => (
                                                    <option key={i} value={s.value}>{s.label}</option>
                                                ))
                                            ) : (
                                                <>
                                                    <option value="general">General Inquiry</option>
                                                    <option value="reservations">Table Reservation</option>
                ...
                <section data-section="map" className="container mx-auto px-4 mt-20 max-w-6xl">
                    <div className="bg-gray-100 rounded-3xl overflow-hidden h-96 w-full relative grayscale hover:grayscale-0 transition-all duration-500">
                        ...
                        <div className="absolute top-4 right-4 bg-white p-4 rounded-xl shadow-lg z-10 hidden md:block">
                            <p className="font-bold text-gray-900">{contactConfig?.contactForm?.mapTitle || storeConfig.name}</p>
                            <p className="text-sm text-gray-500">{contactConfig?.contactForm?.mapAddress || storeConfig.contactInfo?.address?.street || '123 Commerce Blvd'}</p>
                        </div>
                    </div>
                </section>
            )}
