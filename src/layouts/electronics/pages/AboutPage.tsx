...
export function AboutPage({ storeConfig: initialConfig }: AboutPageProps) {
  ...
  const heroImage = aboutConfig?.aboutHeader?.image || aboutConfig?.heroImage || 'https://images.unsplash.com/photo-1498049381960-da565139a979?q=80&w=2071&auto=format&fit=crop';
  ...
  const missionStatement = aboutConfig?.aboutHeader?.description || aboutConfig?.missionStatement;
  const galleryImages = aboutConfig?.aboutContent?.gallerySection || aboutConfig?.gallerySection;

  const values = [
    {
      icon: Award,
      title: "Cutting Edge",
      description: "We bring you the latest innovations and newest gadgets first."
    },
    {
      icon: Users,
      title: "Tech Support",
      description: "Our experts are always here to help you set up and troubleshoot."
    },
    {
      icon: Heart,
      title: "Quality Assured",
      description: "Every product is tested for performance and durability."
    },
    {
      icon: Leaf,
      title: "Eco-Tech",
      description: "Promoting energy-efficient devices and responsible e-waste disposal."
    }
  ];
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12">
            {[
              ...
              { label: "Products", value: storeConfig.products?.length ? `${storeConfig.products.length}+` : "100+" },
              { label: "Years in Tech", value: "12+" },
              { label: "Service Points", value: "15+" }
            ].map((stat, idx) => (
