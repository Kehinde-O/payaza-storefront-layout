...
export function ContactPage({ storeConfig: initialConfig }: ContactPageProps) {
    ...
    // Get contact page configs - check pages.contact first, then fallback to sections
    const contactHeaderConfig = layoutConfig?.pages?.contact?.contactHeader || 
                                 layoutConfig?.sections?.contactHeader;
    const contactFormConfig = layoutConfig?.pages?.contact?.contactForm || 
                               layoutConfig?.sections?.contactForm;
    ...
    return (
        <div className="min-h-screen bg-white pb-20">
            {/* Header Section */}
            {contactHeaderConfig?.show !== false && (
                <section data-section="hero" className="relative bg-gray-900 text-white py-20 overflow-hidden">
                    <div className="absolute inset-0 opacity-20">
                        <Image
                            src={contactHeaderConfig?.backgroundImage || "https://images.unsplash.com/photo-1596524430615-b46475ddff6e?q=80&w=2070&auto=format&fit=crop"}
                    ...
                    <div className="container mx-auto px-4 relative z-10 text-center">
                        <h1 className="text-4xl md:text-6xl font-bold mb-4 tracking-tight">
                            {contactHeaderConfig?.title || "Get in Touch"}
                        </h1>
                        <p className="text-xl text-gray-300 max-w-2xl mx-auto font-light">
                            {contactHeaderConfig?.subtitle || "We'd love to hear from you. Here's how you can reach us."}
                        </p>
                    </div>
                </section>
            )}
            ...
            <div data-section="form" className="container mx-auto px-4 -mt-10 relative z-20 max-w-6xl">
                <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 flex flex-col lg:flex-row">
                    ...
                    <div className="lg:w-2/5 p-8 md:p-12 text-white relative overflow-hidden" style={{ backgroundColor: 'var(--store-primary)' }}>
                        ...
                        <h2 className="text-2xl font-bold mb-8 relative z-10">{contactFormConfig?.infoTitle || "Contact Information"}</h2>
                        <p className="mb-10 text-white/90 leading-relaxed relative z-10 whitespace-pre-line">
                            {contactFormConfig?.infoDescription || "Fill out the form and our team will get back to you within 24 hours."}
                        </p>

                        <div className="space-y-8 relative z-10">
                            {(contactFormConfig?.phone || contactFormConfig?.phoneSecondary) && (
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    {contactFormConfig?.phone && <p className="text-white/80">{contactFormConfig.phone}</p>}
                                    {contactFormConfig?.phoneSecondary && <p className="text-white/80">{contactFormConfig.phoneSecondary}</p>}
                                    {!contactFormConfig?.phone && !contactFormConfig?.phoneSecondary && (
                                        <>
                            ...
                            {(contactFormConfig?.email || contactFormConfig?.emailSecondary) && (
                            <div className="flex items-start gap-4">
                                <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center flex-shrink-0">
                                ...
                                <div>
                                    ...
                                    {contactFormConfig?.email && <p className="text-white/80">{contactFormConfig.email}</p>}
                                    {contactFormConfig?.emailSecondary && <p className="text-white/80">{contactFormConfig.emailSecondary}</p>}
                                    {!contactFormConfig?.email && !contactFormConfig?.emailSecondary && (
                                        <>
                            ...
                            {contactFormConfig?.showHours !== false && contactFormConfig?.hours && (
                            <div className="flex items-start gap-4">
                                <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center flex-shrink-0">
                        ...
                        {contactFormConfig?.showSocialLinks !== false && (
            ...
            {/* Map Section */}
            {contactFormConfig?.showMap !== false && (
