...
export function AboutPage({ storeConfig: initialConfig }: AboutPageProps) {
  ...
  // Get about page configs - check pages.about first, then fallback to sections
  const heroConfig = layoutConfig?.pages?.about?.hero || layoutConfig?.sections?.hero;
  const storyConfig = layoutConfig?.pages?.about?.story || layoutConfig?.sections?.story;
  const statsConfig = layoutConfig?.pages?.about?.stats || layoutConfig?.sections?.stats;
  const contactInfoConfig = layoutConfig?.pages?.about?.['contact-info'] || layoutConfig?.sections?.['contact-info'];
  const valuesConfig = layoutConfig?.pages?.about?.values || layoutConfig?.sections?.values;

  // Hero section data
  const heroImage = heroConfig?.heroImage || 'https://images.unsplash.com/photo-1558769132-cb1aea458c5e?q=80&w=2074&auto=format&fit=crop';
  const pageTitle = heroConfig?.title || "Crafting Experiences, Delivering Joy.";
  const missionStatement = heroConfig?.missionStatement || storeConfig.description;

  // Story section data
  const pageContent = storyConfig?.content || storeConfig.description;
  const gallerySection = storyConfig?.gallerySection || [];

  // Values section data - use config if available, otherwise use defaults
  const valuesItems = valuesConfig?.items || [
    { icon: 'Award', title: "Premium Fabrics", description: "We use only the highest quality materials for our collections." },
    { icon: 'Users', title: "Style for All", description: "Incisive designs that celebrate every individual's unique style." },
  ...
  return (
    <div className="min-h-screen bg-white pb-20 overflow-hidden">
          ...
          <div className="container mx-auto max-w-7xl">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              <div className="relative">
                {gallerySection && gallerySection.length > 0 ? (
                  <div className="grid grid-cols-2 gap-4">
                    {gallerySection.slice(0, 4).map((img: any, i: number) => (
              ...
              <div className="space-y-10">
                ...
                {valuesConfig?.show !== false && valuesItems.length > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    {valuesItems.map((item: any, idx: number) => (
                      <div key={idx} className="flex gap-4 group">
                        <div className="flex-shrink-0">
                          <div className="w-14 h-14 rounded-2xl flex items-center justify-center transition-colors group-hover:bg-gray-100" style={{ backgroundColor: `${primaryColor}10` }}>
                            <DynamicIcon name={item.icon} className="h-7 w-7 transition-transform group-hover:scale-110" style={{ color: primaryColor }} />
                          </div>
                        </div>
                        <div>
      ...
      {contactInfoConfig?.show !== false && (
      <section data-section="contact-info" className="py-24 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              {contactInfoConfig?.title || "Get in Touch"}
            </h2>
            <p className="text-gray-500 max-w-xl mx-auto">
              {contactInfoConfig?.description || "Have questions? We'd love to hear from you. Reach out to our team via email, phone, or visit us at our store."}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="group p-8 rounded-[2rem] bg-gray-50 border border-gray-100 hover:shadow-xl hover:-translate-y-2 transition-all duration-300 text-center">
              ...
              <p className="text-gray-600 leading-relaxed whitespace-pre-line">
                {contactInfoConfig?.address || "123 Store Street\nCommerce City, ST 12345"}
              </p>
            </div>
            ...
            <div className="group p-8 rounded-[2rem] bg-gray-50 border border-gray-100 hover:shadow-xl hover:-translate-y-2 transition-all duration-300 text-center relative overflow-hidden">
              ...
              <p className="text-gray-600 leading-relaxed mb-4">
                {contactInfoConfig?.email || `hello@${storeConfig.slug}.com`}
                {contactInfoConfig?.emailSecondary && (
                  <>
                    <br />
                    {contactInfoConfig.emailSecondary}
                  </>
            ...
            <div className="group p-8 rounded-[2rem] bg-gray-50 border border-gray-100 hover:shadow-xl hover:-translate-y-2 transition-all duration-300 text-center">
              ...
              <p className="text-gray-600 leading-relaxed whitespace-pre-line">
                {contactInfoConfig?.hours || "Mon - Fri: 9am - 6pm\nSat - Sun: 10am - 4pm"}
              </p>
            </div>
          </div>
        </div>
