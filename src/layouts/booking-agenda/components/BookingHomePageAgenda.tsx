...
import { Calendar, Clock, ChevronRight, Star, Search, User, Heart, Filter, MoreHorizontal, X, CheckCircle, Sparkles, Lock } from 'lucide-react';
...
export function BookingHomePageAgenda({ storeConfig: initialConfig }: BookingHomePageAgendaProps) {
  ...
  const services = activeServices.length > 0 
    ...
    : (isPreview && rawServices.length > 0 
        ? rawServices 
        : (isPreview ? [
            { id: 'mock-1', name: 'Premium Haircut', description: 'Expert styling and precision cut', price: 50, duration: 45, categoryId: 'cat-1', status: 'active', currency: 'USD', image: 'https://images.unsplash.com/photo-1560066984-138dadb4c035?w=800' },
            { id: 'mock-2', name: 'Beard Grooming', description: 'Trim and shape with straight razor finish', price: 30, duration: 30, categoryId: 'cat-1', status: 'active', currency: 'USD', image: 'https://images.unsplash.com/photo-1503951914875-452162b0f3f1?w=800' },
            { id: 'mock-3', name: 'Full Facial Spa', description: 'Deep cleansing and hydration treatment', price: 85, duration: 60, categoryId: 'cat-2', status: 'active', currency: 'USD', image: 'https://images.unsplash.com/photo-1540555700478-4be289fbecef?w=800' }
          ] : []));
  ...
  return (
    <div className="h-full min-h-screen font-sans" style={{ backgroundColor: bgPrimary, color: textPrimary }}>
      ...
      <div className="relative z-10 flex flex-col md:flex-row max-w-[1600px] mx-auto backdrop-blur-xl h-full min-h-screen shadow-[0_0_100px_rgba(0,0,0,0.05)]" style={{ backgroundColor: bgOverlay }}>
          ...
          <div className="bg-white/80 backdrop-blur-md border-b border-gray-100/50 sticky top-0 z-20">
             <div className="px-4 md:px-8 py-4 md:py-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div data-section="header">
                   <h1 className="text-2xl font-serif font-bold text-slate-900 mb-1">{layoutConfig?.sections?.header?.title || getTextContent(storeConfig, 'booking_page_title', 'Book an Appointment')}</h1>
                   <p className="text-slate-500 text-sm">{layoutConfig?.sections?.header?.subtitle || getLayoutText(storeConfig, 'booking.pageSubtitle', getTextContent(storeConfig, 'booking_page_subtitle', 'Select a date and service to continue'))}</p>
                </div>
             ...
             <div className="flex overflow-x-auto px-4 md:px-8 pb-4 md:pb-6 gap-3 no-scrollbar scroll-smooth">
                {days.map((day, idx) => (
                   <button 
                     ...
                     className={`flex flex-col items-center justify-center min-w-[70px] p-4 rounded-2xl border transition-all duration-300 ${
                        selectedDate === idx 
      ...
      <div 
        ...
        className="relative rounded-3xl overflow-hidden shadow-2xl shadow-rose-900/20 group cursor-pointer" 
        onClick={() => handleBookClick()}
      >
         ...
         <div className="relative z-10 p-6 md:p-14 text-white max-w-2xl">
            <span className="inline-block px-4 py-1.5 bg-white/20 backdrop-blur-md rounded-full text-xs font-bold uppercase tracking-widest mb-4 md:mb-6 border border-white/30">
               {layoutConfig?.sections?.promoBanner?.badge || getTextContent(storeConfig, 'promo_badge', 'Summer Exclusive')}
            </span>
            <h2 className="text-3xl md:text-5xl font-serif font-bold mb-4 leading-tight">
               {(layoutConfig?.sections?.promoBanner?.title || getTextContent(storeConfig, 'promo_title', 'Total Renewal\nPackage')).split('\n').map((line, i) => (
                 <span key={i}>
                   {line}
                   {i === 0 && <br />}
                 </span>
            ...
            <p className="text-base md:text-lg text-white/90 mb-6 md:mb-8 font-light leading-relaxed max-w-lg">
               {layoutConfig?.sections?.promoBanner?.description || getTextContent(storeConfig, 'promo_desc', 'Indulge in a 90-minute holistic treatment including a full body massage, organic facial, and aromatherapy session.')}
            </p>
            <div className="flex items-center gap-4">
               <Button className="bg-white text-slate-900 hover:bg-rose-50 h-12 px-8 rounded-full font-bold shadow-lg transition-transform hover:scale-105">
                  {layoutConfig?.sections?.promoBanner?.buttonText || getLayoutText(storeConfig, 'booking.bookAppointment', 'Book Now')} • $120
               </Button>
      ...
      <div data-section="services">
         <div className="flex items-end justify-between mb-8">
            <h3 className="text-2xl font-serif font-bold text-slate-900">{layoutConfig?.sections?.services?.title || getLayoutText(storeConfig, 'booking.availableServices', getTextContent(storeConfig, 'services_list_title', 'Available Services'))}</h3>
         ...
         <section data-section="team">
            <div className="flex items-end justify-between mb-8">
               ...
               >
                  {layoutConfig?.sections?.team?.viewAllLabel || getTextContent(storeConfig, 'experts_view_all', 'View All')} <ChevronRight className="h-4 w-4" />
               </Link>
            </div>
             ...
             {(() => {
               ...
               const fallbackTestimonials = [
                 ...
                 },
                 {
                   ...
                   name: 'Marcus Johnson',
                   role: 'Regular Client',
                   quote: "Best service in town. The team is professional and the atmosphere is so relaxing.",
                   rating: 5,
                   order: 2
                 },
                 {
                   ...
                   name: 'Sophia Chen',
                   role: 'Monthly Member',
                   quote: "Consistency is key for me, and they never disappoint. Highly recommend the membership!",
                   rating: 5,
                   order: 3
                 }
               ];
               const testimonials = backendTestimonials.length > 0 ? backendTestimonials : (isPreview ? fallbackTestimonials : []);
