...
export function AboutPage({ storeConfig: initialConfig }: AboutPageProps) {
  ...
  // Header Data
  const header = aboutConfig?.aboutHeader;
  const heroImage = header?.image || aboutConfig?.heroImage || 'https://images.unsplash.com/photo-1559339352-11d035aa65de?q=80&w=1974&auto=format&fit=crop';
  const pageTitle = header?.title || aboutConfig?.title || "Crafting Experiences, Delivering Joy.";
  const missionStatement = header?.description || aboutConfig?.missionStatement || storeConfig.description;
  const showBadge = header?.showBadge !== false;
  const badgeText = header?.badgeText || "Est. 2024";

  // Content Data
  const content = aboutConfig?.aboutContent;
  const sectionLabel = content?.label || "Who We Are";
  const sectionTitle = content?.title || "Our Journey";
  const pageContent = content?.content || aboutConfig?.content;
  const showGallery = content?.showGallery !== false;
  const galleryImages = content?.gallerySection || aboutConfig?.gallerySection;

  // Stats Data
  const stats = aboutConfig?.statsSection;
  const showStatsSection = stats?.show !== false;
  ...
  const values = [
    {
      ...
      description: "We source the finest local ingredients for every dish we prepare."
    },
    {
      icon: Users,
      title: "Community Driven",
      description: "We're proud to be a local favorite and give back to our community."
    },
    {
      icon: Heart,
