...
export function MotivationalLayout({ children, storeConfig }: MotivationalLayoutProps) {
  ...
  return (
    <AlertModalProvider>
      <div className="min-h-screen flex flex-col font-sans" style={cssVariables}>
        {/* Header Wrapper to handle Overlay on Home */}
        {shouldShowHeader && (
          <div className={cn("w-full z-[60]", isHomePage ? "fixed top-0 left-0 right-0" : "relative")}>
            <StoreHeader storeConfig={storeConfig} cartItemCount={cartCount} />
          </div>
        )}
        
        <main className={cn("flex-1 relative", !isHomePage && "pt-0")}>
