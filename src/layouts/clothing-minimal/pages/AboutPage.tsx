...
export function AboutPage({ storeConfig: initialConfig }: AboutPageProps) {
  ...
  // Clothing-minimal-specific hero image
  const heroImage = aboutConfig?.aboutHeader?.heroImage || aboutConfig?.heroImage || 'https://images.unsplash.com/photo-1558769132-cb1aea458c5e?q=80&w=2074&auto=format&fit=crop';

  const pageTitle = aboutConfig?.aboutHeader?.title || aboutConfig?.title || "Crafting Experiences, Delivering Joy.";
  const pageContent = aboutConfig?.aboutContent?.content || aboutConfig?.content || storeConfig.description;
  const missionStatement = aboutConfig?.aboutHeader?.missionStatement || aboutConfig?.missionStatement || storeConfig.description;

  const values = [
    {
      icon: Award,
      title: "Pure Materials",
      description: "We use only the highest quality organic and sustainable fabrics."
    },
    {
      icon: Users,
      title: "Inclusive Design",
      description: "Fashion designed for every body and every style."
    },
    {
      icon: Heart,
      title: "Ethical Craft",
      description: "Our garments are made in ethical workshops with fair wages."
    },
    {
      icon: Leaf,
      title: "Eco-Conscious",
  ...
  return (
    <div className="min-h-screen bg-white pb-20 overflow-hidden">
      {/* Hero Section */}
      {aboutConfig?.aboutHeader?.show !== false && (
      ...
      {/* Our Story */}
      {aboutConfig?.aboutContent?.show !== false && (
          ...
          <div className="container mx-auto max-w-7xl">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              <div className="relative">
                {aboutConfig?.gallerySection && aboutConfig.gallerySection.length > 0 ? (
                  <div className="grid grid-cols-2 gap-4">
                    {aboutConfig.gallerySection.slice(0, 4).map((img, i) => (
      ...
      {/* Stats Section */}
      {aboutConfig?.statsSection?.show !== false && (
      ...
      {aboutConfig?.contactSection?.show !== false && (
