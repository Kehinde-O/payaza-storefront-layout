...
export function ImageWithFallback({ 
  ...
  const checkAndUpdateImageState = (img: HTMLImageElement | null, forceCheck = false) => {
    if (!mountedRef.current || !img) return;
  ...
  useEffect(() => {
    if (!mountedRef.current || !currentSrc) return;
  ...
  if (!currentSrc) {
    ...
    return <ImageSkeleton className={className} aspectRatio={skeletonAspectRatio} />;
  }
  ...
  if (isLoading && currentSrc) {
    ...
    return (
      <div className={cn("relative", className)}>
        ...
        />
        {/* Skeleton overlay */}
        <ImageSkeleton className={className} aspectRatio={skeletonAspectRatio} />
      </div>
    );
  }
  ...
  if (error) {
    if (showSkeletonOnError && !fallbackText) {
      ...
      return (
        <div className={cn("flex flex-col items-center justify-center bg-gray-100 text-gray-400 border border-gray-200", className)} style={{ minHeight: '100px' }}>
          <svg className="w-1/4 h-1/4 max-w-[48px] max-h-[48px] opacity-20" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          {fallbackText && <span className="text-[10px] uppercase tracking-widest font-bold mt-2 opacity-40">{fallbackText}</span>}
        </div>
      );
    }
  }
