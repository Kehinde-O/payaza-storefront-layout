...
import { getGuestUserInfo, saveGuestUserInfo, hasCompleteGuestInfo, GuestUserInfo } from '@/lib/guest-user';
...
export function CheckoutButton({ 
  ...
  const [pendingCheckout, setPendingCheckout] = useState<(() => void) | null>(null);
  ...
  const proceedWithCheckout = async (userInfo?: GuestUserInfo) => {
    if (cart.length === 0) {
  ...
  return (
    <>
      <Button
        ...
        className={className}
      >
        {isLoading || isProcessing ? 'Processing...' : children}
      </Button>
